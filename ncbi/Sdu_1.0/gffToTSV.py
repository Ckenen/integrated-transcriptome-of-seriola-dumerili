...
def main():
    ...
    with open(path1, "rt") as f:
        for line in f:
            ...
            if columns[2] == "gene":
                if "gene_biotype" in attributes.keys():
                    biotypes[attributes["ID"]] = attributes["gene_biotype"]
                else:
                    assert False
            if "Parent" in attributes.keys():
                parents[attributes["ID"]] = attributes["Parent"]
            if "gene" in attributes.keys():
                genes[attributes["ID"]] = attributes["gene"]
    ...
    with open(path2) as f:
        for line in f:
            columns = line.strip("\n").split("\t")
            tid = columns[3]
            gid = parents.get(tid, tid)
            gname = genes.get(gid, "")
            biotype = biotypes.get(gid, "")
            chrom = columns[0]
            start = columns[1]
            end = columns[2]
            strand = columns[5]
