...
rule make_ref_gtf:
    input:
        gtf = "results/assembly/final.gtf.gz"
    output:
        gtf = outdir + "/ref.gtf"
    shell:
        """
...
rule rmat_prep:
    input:
        ...
        gtf = outdir + "/ref.gtf"
    output:
        txt = temp(outdir + "/prep/{sample}.txt"),
        out = temp(directory(outdir + "/prep/{sample}")),
        tmp = directory(outdir + "/prep/{sample}.tmp")
    log:
        log = outdir + "/prep/{sample}.log"
    threads:
        4
    shell:
        """
...
rule rmats_2:
    input:
        ...
        gtf = outdir + "/ref.gtf"
    output:
        txt1 = temp(outdir + "/pairs2/{group1}_vs_{group2}.batch1.txt"),
        txt2 = temp(outdir + "/pairs2/{group1}_vs_{group2}.batch2.txt"),
        tmp = temp(directory(outdir + "/pairs2/{group1}_vs_{group2}.tmp")),
